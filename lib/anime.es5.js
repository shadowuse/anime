...
var anime = (function() {
    ...
    if (from && typeof from === "object" || typeof from === "function")
      for (var keys = __getOwnPropNames(from), i = 0, n = keys.length, key2; i < n; i++) {
        ...
        if (!__hasOwnProp.call(to, key2) && key2 !== except)
          __defProp(to, key2, { get: function(k) {
            return from[k];
          }.bind(null, key2), enumerable: !(desc = __getOwnPropDesc(from, key2)) || desc.enumerable });
      }
    return to;
  };
  ...
  function getPathProgress(path, progress, isPathTargetInsideSVG) {
    ...
    var svg2 = getParentSvg(path.el, path.svg);
    var p = point();
    var p0 = point(-1);
    var p1 = point(1);
    var scaleX = isPathTargetInsideSVG ? 1 : svg2.w / svg2.vW;
    var scaleY = isPathTargetInsideSVG ? 1 : svg2.h / svg2.vH;
    switch (path.property) {
      case "x":
        return (p.x - svg2.x) * scaleX;
      case "y":
        return (p.y - svg2.y) * scaleY;
      case "angle":
  ...
  function normalizePropertyTweens(prop, tweenSettings) {
    ...
    return propArray.map(function(v, i) {
      ...
      if (is.und(obj2.endDelay))
        obj2.endDelay = i === propArray.length - 1 ? tweenSettings.endDelay : 0;
      return obj2;
    }).map(function(k) {
    ...
    transform: function transform(t, p, v, transforms, manual) {
      ...
      if (p === transforms.last || manual) {
        ...
        transforms.list.forEach(function(value, prop) {
          str2 += "".concat(prop, "(").concat(value, ") ");
        });
        t.style.transform = str2;
      }
    }
  };
  ...
  function getInstanceTimings(animations, tweenSettings) {
    ...
    var getTlOffset = function getTlOffset2(anim) {
